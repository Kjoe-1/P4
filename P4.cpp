...
public:
    ...
    // Operator + untuk menambah luasan dua persegi panjang
    ...
    // Operator - untuk mengambil irisan dua persegi panjang
    PersegiPanjang operator-(const PersegiPanjang& other) const {
        if (!isOverlapping(other)) {
            std::cerr << "Error: Persegi panjang tidak saling beririsan." << std::endl;
            return *this;
    ...
    // Operator ++ untuk menggandakan luas persegi panjang
    ...
    // Operator -- untuk mengurangi setengah luas persegi panjang
    PersegiPanjang operator--() {
        double new_length = (xmax - xmin) / sqrt(2);
        double new_width = (ymax - ymin) / sqrt(2);
        double new_xc = (xmin + xmax) / 2;
    ...
    // Method untuk memeriksa apakah dua persegi panjang saling beririsan
    ...
    // Operator == untuk memeriksa apakah dua persegi panjang saling beririsan
...
int main() {
    // Contoh penggunaan
    PersegiPanjang pp1(0.0, 0.0, 4.0, 2.0); // Titik tengah (0,0), panjang 4, lebar 2
    PersegiPanjang pp2(1.0, 1.0, 3.0, 2.0); // Titik tengah (1,1), panjang 3, lebar 2
    PersegiPanjang pp3(6.0, 6.0, 3.0, 2.0); // Titik tengah (6,6), panjang 3, lebar 2
